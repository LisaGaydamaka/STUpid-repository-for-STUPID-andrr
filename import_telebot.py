...
def update_db(message):
    conn = sqlite3.connect(r"C:\Users\lisa\Desktop\ffff\bott.db")
    cursor = conn.cursor()
...
def game(message):
    if not message.text.isdigit():
        return 'Choose the number or /stop the game'
    elif not int(message.text) in range(1, 6):
        return 'Choose the right number'
    else:
        you = int(message.text)
        bott = random.randrange(1, 6)
        no = 'Too bad it was ' + str(bott)
